...
{
    DDRJ=0X00;               //设置为输入
    PERJ=0XC1;               //PJ6、7使能内置上拉、下拉电阻
    PIEJ=0XC1;               //允许PJ6、7中断
    PPSJ=0XC1;               //PJ6、7上升沿触发
}

#pragma CODE_SEG __NEAR_SEG NON_BANKED 
__interrupt 24 void portj(void)
{
    //delay(10);
    if(PIFJ_PIFJ6==1)
    {  
...
{
   LED_WrCmd(0xAE|ucData);
}

void SetChargePump(uchar ucData)
{
   LED_WrCmd(0x8D);
   LED_WrCmd(0x10|ucData);
}
...
{
   LED_WrCmd(0xA0|ucData);
}

void SetCommonRemap(uchar ucData)
{
   LED_WrCmd(0xC0|ucData);
}
...
void LED_P8x16Char(byte x,byte y,uchar ch)
{
  byte c=0,i=0,j=0;
    ...
    LED_SetPos(x,y);    
  	for(i=0;i<8;i++)     
  	  LED_WrDat(F8X16[c*16+i]);
  	LED_SetPos(x,y+1);    
  	for(i=0;i<8;i++)     
  	  LED_WrDat(F8X16[c*16+i+8]);  
  	x+=8;
}   
